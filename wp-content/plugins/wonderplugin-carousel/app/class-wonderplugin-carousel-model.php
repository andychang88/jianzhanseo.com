...
class WonderPlugin_Carousel_Model {
	...
	function generate_body_code($id, $has_wrapper) {
		...
		if ($item_row != null)
		{
			...
			$valOptions = array('visibleitems', 'arrowstyle', 'arrowimage', 'arrowwidth', 'arrowheight', 'navstyle', 'navimage', 'navwidth', 'navheight', 'navspacing');
			...
			if ($data->direction == 'vertical')
				$ret .= ' style="display:none;position:relative;margin:0 auto;width:' . $data->width . 'px;"';
			else if (strtolower($data->responsive) === 'true')
				$ret .= ' style="display:none;position:relative;margin:0 auto;width:100%;max-width:' . $data->width * $data->visibleitems . 'px;"';
			else 
				$ret .= ' style="display:none;position:relative;margin:0 auto;width:' . $data->width * $data->visibleitems . 'px;"';
